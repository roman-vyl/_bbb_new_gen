...
describe("renderWindowController", () => {
  ...
  it("visible_range_changed without pointerdown promotes to user_panning", () => {
    const manager = createChartDataWindowManager();
    manager.reset(100_000);
    manager.buildTailWindow();
  ...
  it("recordBoundaryIntent succeeds after keyboard-only visible_range_changed", () => {
    const manager = createChartDataWindowManager();
    manager.reset(100_000);
    manager.buildTailWindow();
    ...
    controller.dispatch({
      type: "visible_range_changed",
      visible: { from: 0, to: manager.getWindowLength() - 1 },
      anchorTimeSec: 1_700_000_000,
    });
    const recorded = controller.recordBoundaryIntent(
      { from: 0, to: manager.getWindowLength() - 1 },
  ...
  it("idle debounce commits shift after keyboard-only visible_range_changed", () => {
    vi.useFakeTimers();
    const manager = createChartDataWindowManager();
    manager.reset(100_000);
    manager.buildTailWindow();
  ...
  it("programmatic_viewport_start blocks user-pan promotion on visible_range_changed", () => {
    const manager = createChartDataWindowManager();
    manager.reset(100_000);
    manager.buildTailWindow();
  ...
  it("visible_range_changed during applying_shift aborts stale swap", () => {
    const manager = createChartDataWindowManager();
    manager.reset(100_000);
    manager.buildTailWindow();
