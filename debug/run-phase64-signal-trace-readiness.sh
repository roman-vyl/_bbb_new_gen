#!/usr/bin/env bash
# Phase 6.4 signal-trace readiness diagnostic orchestrator.
# Alternates: main-run1 → branch-run1 → main-run2 → branch-run2
#
# Usage: ./debug/run-phase64-signal-trace-readiness.sh
...
run_capture() {
  ...
  (
    cd "$REPO_ROOT/frontend"
    node ../debug/capture-phase64-signal-trace-readiness-smoke.mjs --git-branch "$git_branch" --run "$run"
  )
}
...
if ! git diff --quiet || ! git diff --cached --quiet; then
  ...
  git stash push -u -m "phase64-signal-trace-readiness-diagnostic" 2>/dev/null || git stash push -m "phase64-signal-trace-readiness-diagnostic"
  STASHED=true
fi
...
echo "Artifacts: $REPO_ROOT/debug/reports/phase64-signal-trace-readiness/"
