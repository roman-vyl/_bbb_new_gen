...
Split frontend market resource identity so candles are reusable across variants with the same symbol/timeframe/range while overlays refresh independently. `/api/market/chart-bundle` remains the network source in PR5; cold payload reduction awaits split backend endpoints.
