# anchor-stack-width-setup Specification

## Purpose

EMA-pullback setup component that gates entries when the strategy-level anchor EMA stack is wide enough on the entry bar and was sufficiently expanded within a lookback window, normalized by ATR. Side-neutral (`abs(fast - slow)`); reuses `strategy.anchor_stack` EMAs (no per-setup EMA periods). Opt-in; composes with other setups via AND. Chart shows allowed episodes only (`Width ok` / `Width end` on transitions).
