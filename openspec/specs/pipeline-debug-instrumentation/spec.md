...
4. Pan with render-window shift: window bounds change, slice recomputed, viewport restore scheduled.
5. Signal trace: fetch (existing `api.fetchSignalTrace`), display-cache hit vs miss, chunk merge, slice events and slice HTF for current render window.
...
- **THEN** the debug output includes `wb.render_window.shift` (or equivalent) and chart viewport restore timing with restore `method` metadata when applicable

### Requirement: Signal trace display cache hit and miss are distinguishable

When `VITE_EMA_PIPELINE_DEBUG` is `"true"`, signal-trace load policy and display-cache usage SHALL emit debug marks or timed steps that distinguish at least: network fetch required, display cache covers current render window (hit), and display cache partial miss requiring fetch.

#### Scenario: Display cache hit skips redundant fetch policy

- **WHEN** the render window is already covered by `SignalTraceDisplayCache` for the active cache key
- **THEN** debug output includes a cache-hit mark and `wb.signal_trace_decision` shows a skip or satisfied action without a new `api.fetchSignalTrace` timed entry for that window

#### Scenario: Display cache miss triggers fetch and merge

- **WHEN** the render window is not covered and a signal-trace fetch completes
- **THEN** debug output includes cache-miss (or fetch) visibility, `wb.trace_display.merge_chunk` timing, and slice timings for events and HTF context for the active window
