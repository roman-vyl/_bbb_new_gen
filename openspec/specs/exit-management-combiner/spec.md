...
The `ema_pullback` research runtime SHALL process stateful exit-management rules inside the existing execution flow using compiled entries, compiled exit-policy outputs, profile/context state, and OHLCV. It MUST NOT require a separate orchestration engine beside the current research backtest path.

#### Scenario: Managed runtime consumes compiled inputs
- **GIVEN** a strategy spec with `trade_management.exit_management` rules
...
- **GIVEN** a strategy spec without `trade_management.exit_management` rules
- **WHEN** the backtest runs
