...
- [ ] 3.8 STOP FOR REVIEW before adding parity logic.
...
- [ ] 4.3 Implement market load lifecycle parity in `marketLoadRuntime.ts` with abort, generation, in-flight dedupe, status/error, and revision semantics.
- [ ] 4.4 Implement display bundle composition parity in `marketBundleRuntime.ts`, including focus fallback while expanded coverage loads.
- [ ] 4.5 Add old-vs-new debug comparison for market identity, focus window, coverage window, fetch target, cache range/count, bundle source, and market status.
- [ ] 4.6 Verify cold market output is not empty in shadow/debug mode.
- [ ] 4.7 Verify there is no duplicate active market fetch owner and no second `marketResourceCache` implementation.
- [ ] 4.8 Run market helper and runtime parity tests.
- [ ] 4.9 STOP FOR REVIEW before display/render/viewport parity.
...
- [ ] 5.4 Implement `interactionRuntime.ts` and `panRuntime.ts` parity for pointer/wheel/programmatic/visible-range dispatch and coverage expansion intents.
...
- [ ] 5.7 Verify no empty chart gaps in shadow output.
...
- [ ] 5.10 STOP FOR REVIEW before trace/events/overlays parity.
...
- [ ] 6.7 Verify new runtime can produce a complete `ChartRuntimeOutput` in shadow/debug mode.
...
- [ ] 6.11 STOP FOR REVIEW before atomic Chart tab cutover.
...
- [ ] 7.9 STOP FOR REVIEW before deleting old provider runtime.
...
- [ ] 8.17 STOP FOR REVIEW before final cleanup.
...
- [ ] 9.8 Mark OpenSpec tasks complete only after final verification evidence is recorded.
