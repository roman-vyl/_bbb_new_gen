...
When `confirm_bars > 1`, it MUST require fast on the adverse side of slow for `confirm_bars` consecutive **base bars** (long: `fast < slow`; short: `fast > slow`). This is confirmed EMA order loss, not a repeated cross event each bar.
...
#### Scenario: Adverse side hold on base bars

- **GIVEN** `confirm_bars=3` and three consecutive base bars with `fast < slow` (long)
- **WHEN** evaluated on the third bar with `side="long"`
- **THEN** that bar is `True` without requiring a new cross on each bar
