...
`role` SHALL be a string semantic category used for styling and layer toggles. v1 of this change uses `entry_block` and `exit_signal`. The contract MUST allow additional roles without breaking changes, including at minimum:
...
### Requirement: Migration slice emitters limited to two existing RSI components

In **this change only**, research MUST implement event emitters for these two existing catalog components (first proof of the universal contract):
...
Research MUST NOT add emitters for other components **in this change** (counter-candle blocker, EMA exits, context gates, setup/trigger).
...
#### Scenario: Non-implemented component produces no events in migration slice
...
Partial spans are acceptable in v1: when the visible window intersects the middle or end of a blocked run, only `span_end` (or only `span_start`) MAY appear — this is expected and MUST NOT be treated as a data bug.
...
**Reason**: Replaced by semantic `component_events[]` with `event_type` vocabulary.

**Migration**: Consumers MUST read `component_events` instead of `component_event_markers`. Field rename is breaking; deploy research_api and frontend together.

### Requirement: Component event data is always one record per blocked or active base bar

**Reason**: Blocked regimes are represented by `span_start` / `span_end` pairs (plus optional `source`), not dense per-bar duplication.

**Migration**: Emitters detect contiguous runs on aligned base index; tests expecting N markers for N blocked bars MUST expect span boundaries instead.

### Requirement: Chart v1 uses dense rendering mode

**Reason**: Dense 1:1 per-bar markers contradicted universal semantic events; rendering follows `event_type` at span boundaries and points.

**Migration**: Remove dense-mode chart tests; validate span boundary markers and optional future span shading instead.
