...
- [ ] 1.2 Add a typed setup spec/config model for `ema_bounce_counter_setup` with validation for base timeframe EMA params, `max_bounces`, `raw_touch_mode`, `touch_lookback_bars`, and trend confirmation bars
...
- [ ] 2.1 Update feature planning to request the setup's base timeframe fast, anchor, and slow EMA features through existing EMA feature mechanisms
...
- [ ] 2.5 Add unit tests for trend episode reset, long/short armed logic, raw touch detection, pending-window collapse, inclusive lookback timing, end-of-lookback count increment, final-active-lookback raw touch ignore, and post-limit setup blocking
...
- [ ] 3.5 Add component event tests for shared `span_id`, inclusive base-index `span_end`, final-active-lookback raw touch ignore, `role: setup`, `feature_family: ema`, and metadata-only EMA/counter details
