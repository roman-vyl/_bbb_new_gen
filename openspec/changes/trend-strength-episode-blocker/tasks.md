...
- [x] 2.1 Implement `trend_strength_episode_blocker` and `trend_strength_episode_blocker_trace` in `components/blockers.py` (backward scan for **most recent qualifying** ADX/DMI bar—**not** local ADX max / argmax; freshness, current ADX floor, opposite DI flip, EMA-stack guard)
- [x] 2.2 Register component in `components/registry.py` with description and trace func
- [x] 2.3 Add `blocker_trend_strength_episode(...)` to `component_builders.py`; wire `execution/signals.py` column bindings (ADX/DI + anchor stack EMAs)
...
- [x] 3.2 Optional: snapshot entry-bar trend strength fields on closed `trade_records` in report builder
...
- [x] 3.5 Unit tests: long/short symmetry, `no_recent_adx_peak`, `peak_too_old`, `current_adx_too_low`, `opposite_di_flip`, `ema_stack_direction_broken`, allow path with faded ADX after recent peak
