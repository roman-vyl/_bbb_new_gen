...
### Requirement: Exit policy owns HTF context provider configuration

**Reason**: Provider configuration belongs at strategy instance scope so multiple consumers can share one HTF stack without duplicating fields or coupling to exit policy.

**Migration**: Move fields from `trade_management.exit_policy.context` to `strategy.contexts.<context_ref>` and add `trade_management.exit_policy.context_consumption`. Use a **one-off migration script** or manual edit for stored experiments — not runtime dual-read or loader shim.
