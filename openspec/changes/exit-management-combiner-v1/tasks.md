...
- [ ] 2.8 Add focused unit tests for triggered, never-triggered, no-rule, profile override, always-on fallback, short-side, and same-bar old-stop cases — **partial** (`test_exit_management.py`: validation, resolve, one long BE bar loop, internals filter; not full matrix).
...
- [x] 4.5 Add report tests for triggered, never-triggered, no-management, profile override source, and always-on fallback source — `test_build_managed_trade_records_break_even_exit_reason`, `test_resolve_never_triggered_break_even_diagnostics`, fixture load; profile source covered via fixture shape.
...
- [x] 5.5 Add tests for trigger-bar old effective stop, next-bar promoted stop, API passthrough, and legacy trace payload compatibility — `_attach_exit_management_internals` unit test; `test_to_contract_preserves_exit_management_internals`; legacy empty `internals` unchanged in existing trace tests.
...
- [x] 8.3 Run targeted frontend tests for Composer authoring and selected trade diagnostics — `exitManagementBarInspector.test.ts`, `composerExitManagement.test.ts`, trade diagnostics vitest.
- [x] 8.4 Run existing no-management regression tests — covered by `test_run_strategy_spec_without_exit_management_skips_managed_loop` and `test_exit_management.py` validation suite.
...
- [x] Managed path `exit_reason` / `exit_kind` for `break_even:*` exits; chart marker **BE** (`chartMarkers.ts`).
- [x] Chart Bar Inspector: per-bar **Exit management (break-even)** block from `long`/`short.internals.exit_management` (`exitManagementBarInspector.ts`, `ChartBarInspector.tsx`); vitest in `exitManagementBarInspector.test.ts`.
