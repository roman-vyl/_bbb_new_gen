...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="EMA pullback research runner for active StrategySpec."
    )
    p.add_argument("--symbol", default=DEFAULT_EXECUTION_CONFIG.symbol, help="Symbol in DB")
    p.add_argument("--tf", default=DEFAULT_EXECUTION_CONFIG.timeframe, help="Timeframe")
    p.add_argument(
        "--db-path",
        type=Path,
        default=None,
...
def config_from_args(args: argparse.Namespace) -> ExecutionConfig:
    return ExecutionConfig(
        family=DEFAULT_EXECUTION_CONFIG.family,
