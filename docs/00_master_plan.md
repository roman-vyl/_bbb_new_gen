...
- **Phase 5 — Indicator Framework (если решено вводить).** После MVP отдельно решают, нужны ли **вообще** backend indicators в движке. Если да — в системе появляются «настоящие» индикаторы: EMA/RSI/ATR/MACD/Bollinger живут в БД с явным статусом готовности, считаются единым контрактом, без локальных формул в research-скриптах. Если нет — эта фаза не стартует автоматически; roadmap уточняется отдельно.
...
- **Что появляется (общими словами).** Только `research/ema_smoke.py` + тесты smoke-хелперов и границ фазы; команда `python research/ema_smoke.py`. Скрипт читает свечи через `range_get`, считает EMA(20)/EMA(50) локально `pd.ewm(..., adjust=False)`, стратегия — EMA crossover, собирает `vbt.Data`, печатает метрики. `pandas`/`numpy`/`vectorbt` — optional research extras. Результаты бэктеста в SQLite не пишутся. Strategy Constructor (`docs/strategy_constructor_master_plan.md`) — не scope Phase 4.
...
- **Цель.** **Если** принято решение вводить backend indicators: проект перестаёт опираться только на локальные формулы в research для тех индикаторов, которые должны жить в движке. Появляется индикаторный слой как часть core: единый контракт, единая точка регистрации, явный статус готовности значения.
...
4. **Decision gate закрыт.** Если у фазы есть ADR-зависимость (ADR-001/002 для Phase 5 **когда она запланирована**, ADR-003 для Phase 6, ADR-004 для Phase 8), ADR должен быть написан и принят **до** старта реализации фазы.
