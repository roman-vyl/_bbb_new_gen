...
## 17. Один execution combiner — не два параллельных loop

Исторически существовали два bar-by-bar execution path:

```text
run_managed_bar_loop        — legacy R-trigger break_even_stop (always_on / profiles)
run_managed_execution_loop  — v2 ManagedExitProvider + ExitArbitrator
```

Оба владели open/hold/close — это и было «развоение ответственности».

**Сейчас:** единый `run_execution_combiner_loop` (`execution_combiner.py`) владеет bar sequencing для всех managed paths. Адаптеры подключают только exit/state логику:

```text
LegacyBreakEvenExitAdapter   — старый R-trigger BE (pending_stop N+1, classify SL/TP/signal)
ManagedExitProviderAdapter   — v2 provider + arbitrator
```

Нормативный порядок бара (общий для обоих адаптеров):

```text
1. position_was_open_at_bar_start = snapshot
2. adapter.on_bar_open          — legacy: promote pending_stop
3. adapter.try_close_at_bar_open
4. entry только если NOT position_was_open_at_bar_start
5. adapter.on_end_of_bar        — v2: snapshot N+1; legacy: BE trigger → pending
```

`backtest.py` маршрутизирует через `_needs_execution_combiner()` → один `_run_execution_combiner_strategy_spec()`. Legacy BE combiner больше не отдельный execution owner — тонкая обёртка над combiner + adapter.
